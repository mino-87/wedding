(() => {
  ...
  async function playSurpriseWithSound(){
    ...
    if(!button){
      ...
      button.id = 'soundStartFallback';
      button.type = 'button';
      button.className = 'primary';
      button.textContent = document.documentElement.lang === 'en' ? 'Play surprise with sound' : 'شغّل المفاجأة بالصوت';
      surprise.insertAdjacentElement('afterend',button);
      button.addEventListener('click',async()=>{
        try{surprise.muted=false;surprise.volume=1;await surprise.play();button.remove();}catch{}
      });
    }
  }
  ...
  function loop(){
    ...
    try{
      ...
      const result = detector.detectForVideo(smileVideo,now);
      ...
      const holdMs = Number(config.smileHoldMs || 3000);
      if(smile >= threshold){
        if(!smileSince) smileSince = now;
        const held = now - smileSince;
        const pct = Math.min(100,(held/holdMs)*100);
        setMeter(pct);
        const remaining = Math.max(0,Math.ceil((holdMs-held)/1000));
        if(status) status.textContent = remaining > 0
          ? (document.documentElement.lang === 'en' ? `Hold the smile… ${remaining}` : `ثبت الضحكة… ${remaining}`)
          : (document.documentElement.lang === 'en' ? 'Perfect. Opening…' : 'تمام… بنفتح الدعوة');
        if(held >= holdMs){unlock();return;}
      }else{
        smileSince = 0;
        setMeter(0);
        if(status) status.textContent = document.documentElement.lang === 'en' ? 'Smile and hold it for 3 seconds' : 'ابتسم وخليك ثابت 3 ثواني';
      }
    }catch{}
  ...
  async function startScan(e){
    ...
    try{
      ...
      console.error(err);
      setMeter(0);
      if(status) status.textContent = document.documentElement.lang === 'en' ? 'Camera could not start. Check permission and try again.' : 'الكاميرا ما اشتغلتش. راجع صلاحية الكاميرا وحاول تاني.';
